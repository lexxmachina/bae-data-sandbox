...
with open(input_file,'rb') as csvfile:
        ...
        for row in csvreader:
                ...
                if linecount > 1:
                        row[0] = datetime.datetime.fromtimestamp(int(row[0])).strftime('%Y-%m-%dT%H:%M:%S')
                        newdate = datetime.datetime.strptime(row[0], dateformatstr)
                        if linecount == 2:
                                prevdate = newdate
                                date = newdate
                        else :
